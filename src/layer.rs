use crate::{
  get_value, get_value_int, get_value_string, read_dict, read_int, Bounded, Chunk, Image,
};
...
impl Layer {
  pub fn new(bytes: &mut Vec<u8>) -> Self {
    ...
    let block_count = read_int(bytes);
    let block_length = block_count * 20 + 4;

    for _ in 0..block_count {
      blocks.push(Chunk::new(bytes));
    }

    let dict = {
      if block_length < length {
        read_dict(bytes, length - block_length)
      } else {
        Default::default()
      }
    };
